...
if tsl_normalize:
    tsla_obs['SC_stdev'] = (tsla_obs['SC_stdev'] - tsla_obs['glacier_DEM_min']) / (tsla_obs['glacier_DEM_max'] - tsla_obs['glacier_DEM_min'])
...
class spot_setup:
    ...
    def objectivefunction(self, simulation, evaluation, params=None):
        ...
        if not self.obj_func:
            ...
            #print(sigma_tsla)
            #print(sim_tsla)
            rmse = (((eval_tsla - sim_tsla)/(sigma_tsla**2))**2).mean()**.5
...
mcmc = psample(obs=(tsla_obs,geod_ref), count=1, rep=500, algorithm='mcmc')
