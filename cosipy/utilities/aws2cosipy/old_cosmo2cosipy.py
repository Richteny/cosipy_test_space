...
def set_zero_field_memmap(filestr: str, 
    time_index: int, lat_index: int, lon_index: int
) -> np.ndarray:
    """Initialise array in storage instead of memory."""
    field = np.memmap(filestr, dtype=np.float64,
                      mode='w+', shape=(time_index, lat_index, lon_index))
    return field
...
def create_2D_input(
    ...
    #set zero field with memmap to allocate into space instead of memory - developmental
    T_interp = set_zero_field_memmap("memmappedT.dat", time_index, lat_index, lon_index)
    RH_interp = set_zero_field_memmap("memmappedRH.dat", time_index, lat_index, lon_index)
    ...
    if _cfg.names["RRR_var"] in df:
        ...
        RRR_interp = set_zero_field_memmap("memmappedRRR.dat", time_index, lat_index, lon_index)

    if _cfg.names["SNOWFALL_var"] in df:
        SNOWFALL = df[_cfg.names["SNOWFALL_var"]]  # Snowfall
        #SNOWFALL_interp = set_zero_field(time_index, lat_index, lon_index)
        SNOWFALL_interp = set_zero_field_memmap("memmappedSF.dat", time_index, lat_index, lon_index)

    if _cfg.names["LWin_var"] in df:
        LW = df[_cfg.names["LWin_var"]]  # Incoming longwave radiation
        #LW_interp = set_zero_field(time_index, lat_index, lon_index)
        LW_interp = set_zero_field_memmap("memmappedLW.dat", time_index, lat_index, lon_index)
    ...
    #check what lapse rate variables are in df
    if 'lr_t2m' in df:
        lapse_T = df['lr_t2m']
    else:
        lapse_T = np.zeros_like(T2)
    if 'lr_rh2' in df:
        lapse_RH = df['lr_rh2']
    else:
        lapse_RH = np.zeros_like(RH2)
    if ('lr_lwin' in df) and (_cfg.names["LWin_var"] in df):
        lapse_LW = df['lr_lwin']
    ...
    for t in range(time_index):
        ...
        #)
        T_interp[t, :, :] = set_bias_local(
            data=(T2[t]), lapse_rate=lapse_T[t], altitude=altitude, limit=False
        )
        #RH_interp[t, :, :] = set_bias(
        #    data=RH2[t], lapse_type="lapse_RH", altitude=altitude, limit=False
        #)
        RH_interp[t, :, :] = set_bias_local(
            data=(RH2[t]), lapse_rate=lapse_RH[t], altitude=altitude, limit=False
        )
        U_interp[t, :, :] = U2[t]
        ...
        if _cfg.names["LWin_var"] in df:
            #LW_interp[t, :, :] = LW[t]
            LW_interp[t, :, :] = set_bias_local(
                data=(LW[t]), lapse_rate=lapse_LW[t], altitude=altitude, limit=True
            )
