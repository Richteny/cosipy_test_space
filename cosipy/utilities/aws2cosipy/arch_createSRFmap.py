...
def process(static_2d_path, target_1d_path, output_path, output_2d_path, band_width):
    ...
    if abs(res_meters - 30.0) > 5.0:
        print(f"Warning: DEM resolution is {res_meters:.1f}m. This tool was "
              f"with L=50m will scan only {int(np.ceil(50/res_meters))} pixels at this resolution. Results may be unreliable.")
    ...
    if "MASK" in ds_2d:
        ...
        if len(glacier_vals) > 0:
            ...
            #damping_factor = 0.9
            srf_normalized = np.clip(srf_2d / glacier_mean, 0.2, 1.6) #(damping_factor * srf_norm) + (1.0 - damping_factor)
            n_reclipped = np.sum( (srf_2d/glacier_mean < 0.2) | (srf_2d/glacier_mean > 1.6))
            if n_reclipped > 0:
                print(f"Note: {n_reclipped} pixels were re-clipped after normalisation. Glacier Mean SRF was {glacier_mean:.3f}).")
        else:
