...
class spot_setup:
    ...
    param = lr_T, lr_RRR, RRR_factor, alb_ice, alb_snow = [
        Uniform(low=-0.0064, high=-0.0057),  # lr_temp
        Uniform(low=0, high=0.00013),  # lr_prec
        Uniform(low=0.63, high=0.68),  # RRR factor
        Uniform(low=0.2, high=0.35),  # alb ice
        Uniform(low=0.6, high=0.9),  # alb snow
        ]
...
def psample(obs, rep=10, dbname='cosipy_par_smpl', dbformat="csv", interf=4, freqst=2, ngs=2,
            algorithm='mcmc', name_prefix= "tsla_test_mza_namelist_", savefig=True):

    
    setup = spot_setup(obs)
