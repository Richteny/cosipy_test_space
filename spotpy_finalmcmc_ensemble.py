...
class spot_setup:
    ...
    def simulation(self, x):
        ...
        sim_mb, sim_tsla = runcosipy(RRR_factor=x.RRR_factor, alb_ice = x.alb_ice, alb_snow = x.alb_snow, alb_firn = x.alb_firn,
                   ...
                   roughness_ice = x.roughness_ice, count=self.count)
...
def psample(obs, rep=10, count=None, dbname='MCMC_finalensemble', dbformat="csv", algorithm='mc'):
    ...
    sampler = alg_selector[algorithm](setup, dbname=dbname, dbformat=dbformat, db_precision=np.float64, random_state=42,save_sim=False)
    sampler.sample(rep)
