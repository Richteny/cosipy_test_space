...
def main(lr_T=0.0, lr_RRR=0.0, lr_RH=0.0, RRR_factor=Constants.mult_factor_RRR, alb_ice=Constants.albedo_ice,
    ...
    times = datetime.now()
    if Config.point_evaluation is True:
        ## Create daily mean albedo, daily mean LWout, daily mean SFC
        daily_means = IO.get_result()[['TOTALHEIGHT','LWout','ALBEDO']].sel(time=slice("2003-10-01","2004-09-30")).resample(time="1D").mean()
        daily_lwo = daily_means['LWout'].values.flatten()
    ...
    return (daily_lwo, daily_albedo, daily_sfc)
