...
#    alb_ice  = Uniform(low= 0.18, high=0.4)
#    alb_snow = Uniform(low=0.65, high=0.9)
#    alb_firn = Uniform(low=0.4, high=0.65)
#    albedo_aging = Uniform(low=2, high=23)
#    albedo_depth = Uniform(low=2, high=9)
#    center_snow_transfer_function = Uniform(low=0, high=2)
#    roughness_fresh_snow = Uniform(low=0.15, high=0.3)
#    roughness_ice = Uniform(low=0.7, high=0.27)
#    roughness_firn = Uniform(low=2, high=6)
...
def psample():
    ...
    setup = spot_setup()
    sampler = spotpy.algorithms.fast(setup, dbname=name, dbformat='csv',db_precision=np.float32)
    sampler.sample(rep)
