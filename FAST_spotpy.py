...
class spot_setup:
    ...
    def objectivefunction(self, simulation, evaluation, params=None):
        if not self.obj_func:
            ...
            loglike_mb = ( 1 / np.sqrt( (2*np.pi* (sigma_mb**2) ) ) ) * np.exp( (-1* ( (eval_mb-sim_mb) **2 ) / (2*sigma_mb**2)))
            loglike_tsla = ( 1 / np.sqrt( (2*np.pi* (sigma_tsla**2) ) ) ) * np.exp( (-1* ( (eval_tsla-sim_tsla) **2 ) / (2*sigma_tsla**2)))
