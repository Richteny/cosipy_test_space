...
alb_obs_data = xr.open_dataset("/data/scratch/richteny/Ren_21_Albedo/HEF_processed_HRZ-20CC-filter_albedos_old.nc")
...
if __name__ == "__main__":
    ...
    with pm.Model() as model:
        ...
        #ice = pt.constant(0.18) #mean of 3sigma ensemble
        ...
        #sigma_alb_winter = pm.HalfNormal("sigma_alb_winter", sigma=0.01)
        #sigma_alb_summer = pm.HalfNormal("sigma_alb_summer", sigma=0.03)
        ...
        alb_obs = pm.Normal("alb_obs", mu=mu_alb, sigma=alb_obs_unc, observed=alb_data, shape=mu_alb.shape[0])
        ...
        loglike_mb_std = pm.logp(mb_obs, geod_data)  # Mass balance log-likelihood
        loglike_tsl_std = pm.logp(tsl_obs, tsl_data).mean() # Snowline log-likelihood (vector)
        loglike_alb_std = pm.logp(alb_obs, alb_data).mean()
        #RENAME above
        # ------------------------------------------------------------------------------
