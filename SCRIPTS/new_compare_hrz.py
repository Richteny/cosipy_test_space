...
# All three inputs (HGT, SVF, T2, G) must share the same lat coordinate.
# We use xarray's .sel() to align them explicitly rather than relying on
# ravel() order, which can differ between the HRZ and forcing files.

# Open both files as xarray datasets, aligned on their lat coordinate
...
# Use the forcing file's lat as the common coordinate
lat_coord = ds_forc["lat"]
hgt_xr    = ds_forc["HGT"]                   # (lat,)
msk_xr    = (ds_forc["MASK"] == 1) if "MASK" in ds_forc else xr.ones_like(hgt_xr, dtype=bool)

melt_mask_t  = ds_forc.time.dt.month.isin([5, 6, 7, 8, 9])
T_mean_xr    = ds_forc["T2"].sel(time=melt_mask_t).mean("time")   # (lat,)
G_mean_xr    = ds_forc["G"].sel(time=melt_mask_t).mean("time")    # (lat,)
ds_forc.close()

# SVF: re-index onto forcing lat grid to guarantee alignment
if "svf" in hrz_new_sq:
    svf_xr = hrz_new_sq["svf"].sel(lat=lat_coord, method="nearest")
    print(f"  SVF loaded from HRZ file: "
          f"min={float(svf_xr.min()):.3f} mean={float(svf_xr.mean()):.3f}")
else:
    print("  WARNING: 'svf' not found in new HRZ file — using linear proxy")
    svf_xr = 0.97 - (3700 - hgt_xr) / (3700 - 2440) * (0.97 - 0.72)
    svf_xr = svf_xr.clip(0.72, 0.97)

# Terrain temperature (Prinz et al. 2016 Eq. 6): T_R = T_air + 0.01 × G
T_R_xr = T_mean_xr + K_SOLAR_HEAT * G_mean_xr   # (lat,) in K

terrain_lw_xr = (1.0 - svf_xr) * EPS_ROCK * SIGMA * T_R_xr**4  # (lat,) W m⁻²

# Convert to numpy, apply glacier mask — now all arrays are lat-aligned
msk_f         = msk_xr.values.astype(bool)
hgt_f         = hgt_xr.values
hgt_lw        = hgt_f[msk_f]
tlw_px        = terrain_lw_xr.values[msk_f]      # W m⁻² per glacier pixel
tlw_MJ        = tlw_px * W_TO_MJ                  # MJ m⁻² a⁻¹
tlw_mwe       = tlw_px * melt_hours_per_year * DT / (L_F * RHO_W)  # m w.e. a⁻¹
tlw_prof      = tlw_px                             # alias for reagg calls

mean_T_R = float(T_R_xr.mean())
print(f"  Mean terrain temp (Prinz): {mean_T_R - 273.15:.1f}°C")
print(f"  Terrain LW at tongue  (~2440-2500m): "
      f"{float(terrain_lw_xr.where(hgt_xr < 2500).mean()):.1f} W m⁻²")
print(f"  Terrain LW at mid-glacier (~3000m): "
      f"{float(terrain_lw_xr.where((hgt_xr > 2950) & (hgt_xr < 3050)).mean()):.1f} W m⁻²")
...
ax.axhline(ELA, color="grey", ls=":", lw=1, label=f"ELA ~{ELA:.0f} m")
